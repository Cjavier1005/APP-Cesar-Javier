...
const DEFAULT_CONFIG={cuotaReunion:4, cuotaMensual:25, meses:['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre']};
...
function load(){ const raw=localStorage.getItem(LS_KEY); if(raw){ try{ Object.assign(state,JSON.parse(raw)); }catch{} } renderAll(); }

function renderSelects(){ $('#reunionPersona').innerHTML=state.personas.map(p=>`<option value="${p.id}">${p.nombre}</option>`).join(''); $('#reunionMes').innerHTML=state.config.meses.map(m=>`<option>${m}</option>`).join(''); $('#mensualFamilia').innerHTML=state.familias.map(f=>`<option value="${f.id}">${f.nombre}</option>`).join(''); $('#mensualMes').innerHTML=$('#reunionMes').innerHTML; const sm=$('#saldoManual'); const cb=$('#saldoLock'); if(sm&&cb){ sm.value=state.saldoManual||0; cb.checked=!!state.saldoBloqueado; sm.disabled=cb.checked; } }
...
// CONFIG
$('#guardarCfg')?.addEventListener('click',()=>{ const cr=Number($('#cfgCuotaReunion').value||state.config.cuotaReunion); const cm=Number($('#cfgCuotaMensual').value||state.config.cuotaMensual); const mesesTxt=$('#cfgMeses').value.trim(); const meses= mesesTxt? mesesTxt.split(',').map(x=>x.trim()).filter(Boolean): state.config.meses; state.config={cuotaReunion:cr, cuotaMensual:cm, meses}; save(); showToast('Configuración guardada'); });
